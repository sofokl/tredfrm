...
BYTE IdOutCmd [sizeof (diskid::SENDCMDOUTPARAMS) + IDENTIFY_BUFFER_SIZE - 1];
...
int ReadPhysicalDriveInNTWithAdminRights (void)
{
   ...
   for (drive = 0; drive < MAX_IDE_DRIVES; drive++)
   {
      ...
      swprintf_s(driveName, L"\\\\.\\PhysicalDrive%d", drive);
      ...
      else
      {
         ...
         else
         {
            ...
            if ( DoIDENTIFY (hPhysicalDriveIOCTL,
                       &scip,
                       (diskid::PSENDCMDOUTPARAMS)&IdOutCmd,
                       (BYTE) bIDCmd,
                       (BYTE) drive,
                       &cbBytesReturned))
            {
               ...
               USHORT *pIdSector = (USHORT *)
                             ((diskid::PSENDCMDOUTPARAMS) IdOutCmd) -> bBuffer;
...
int ReadPhysicalDriveInNTUsingSmart (void)
{
   ...
   for (drive = 0; drive < MAX_IDE_DRIVES; drive++)
   {
      ...
      {
         diskid::GETVERSIONINPARAMS GetVersionParams;
         DWORD cbBytesReturned = 0;
         ...
         if ( ! DeviceIoControl (hPhysicalDriveIOCTL, SMART_GET_VERSION,
                   NULL,
                   0,
                   &GetVersionParams, sizeof (diskid::GETVERSIONINPARAMS),
                   &cbBytesReturned, NULL) )
         {
#ifdef PRINTING_TO_CONSOLE_ALLOWED
            if (PRINT_DEBUG)
         ...
         else
         {
            ...
            ULONG CommandSize = sizeof(diskid::SENDCMDINPARAMS) + IDENTIFY_BUFFER_SIZE;
            diskid::PSENDCMDINPARAMS Command = (diskid::PSENDCMDINPARAMS) malloc (CommandSize);
...
#define ID_CMD          0xEC            // Returns ID sector for ATA
            ...
            if ( ! DeviceIoControl (hPhysicalDriveIOCTL,
                                    SMART_RCV_DRIVE_DATA, Command, sizeof(diskid::SENDCMDINPARAMS),
                                    Command, CommandSize,
                                    &BytesReturned, NULL) )
            {
                   // Print the error
            ...
            else
            {
                ...
                DWORD diskdata [256];
                USHORT *pIdSector = (USHORT *)
                             (PIDENTIFY_DATA) ((diskid::PSENDCMDOUTPARAMS) Command) -> bBuffer;
...
int ReadPhysicalDriveInNTWithZeroRights (void)
{
   ...
   for (drive = 0; drive < MAX_IDE_DRIVES; drive++)
   {
      ...
      {
         diskid::STORAGE_PROPERTY_QUERY query;
         DWORD cbBytesReturned = 0;
         char local_buffer [10000];

         memset ((void *) & query, 0, sizeof (query));
         query.PropertyId = diskid::StorageDeviceProperty;
         query.QueryType = diskid::PropertyStandardQuery;
         ...
         if ( DeviceIoControl (hPhysicalDriveIOCTL, IOCTL_STORAGE_QUERY_PROPERTY,
                   ...
                   & cbBytesReturned, NULL) )
         {
             diskid::STORAGE_DEVICE_DESCRIPTOR * descrip = (diskid::STORAGE_DEVICE_DESCRIPTOR *) & local_buffer;
             ...
             {
                diskid::DISK_GEOMETRY_EX * geom = (diskid::DISK_GEOMETRY_EX * ) & local_buffer [0];
#ifdef PRINTING_TO_CONSOLE_ALLOWED
...
BOOL DoIDENTIFY (HANDLE hPhysicalDriveIOCTL, diskid::PSENDCMDINPARAMS pSCIP,
                 diskid::PSENDCMDOUTPARAMS pSCOP, BYTE bIDCmd, BYTE bDriveNum,
                 PDWORD lpcbBytesReturned)
{
   ...
   return ( DeviceIoControl (hPhysicalDriveIOCTL, DFP_RECEIVE_DRIVE_DATA,
               (LPVOID) pSCIP,
               sizeof(diskid::SENDCMDINPARAMS) - 1,
               (LPVOID) pSCOP,
               sizeof(diskid::SENDCMDOUTPARAMS) + IDENTIFY_BUFFER_SIZE - 1,
...
#define  SENDIDLENGTH  sizeof (diskid::SENDCMDOUTPARAMS) + IDENTIFY_BUFFER_SIZE
   ...
   for (controller = 0; controller < 16; controller++)
   {
      ...
      if (hScsiDriveIOCTL != INVALID_HANDLE_VALUE)
      {
         ...
         for (drive = 0; drive < 2; drive++)
         {
            ...
            diskid::SENDCMDINPARAMS *pin = (diskid::SENDCMDINPARAMS *) (buffer + sizeof (SRB_IO_CONTROL));
            DWORD dummy;
            ...
            strncpy_s ((char *) p -> Signature, 10, "SCSIDISK", 8);
            ...
            if (DeviceIoControl (hScsiDriveIOCTL, IOCTL_SCSI_MINIPORT,
                                 ...
                                 &dummy, NULL))
            {
               diskid::SENDCMDOUTPARAMS *pOut = (diskid::SENDCMDOUTPARAMS *) (buffer + sizeof (SRB_IO_CONTROL));
...
//         done = ReadDrivePortsInWin9X ();
       printf ("\nError reading the drive IDs. Maybe this is Win9X\n");
   }
...
static void PrintMACaddress(unsigned char MACData[])
{
   ...
   WriteConstantString ((char*)"MACaddress", string);
}
...
SYSTEMIDSDATA sprintf_sn(bool getBirdMC)
{
