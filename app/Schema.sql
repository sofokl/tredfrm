...
CREATE VIEW "V01" AS SELECT 
	...
	as InOrder 
	FROM PRODUCTS
	LEFT JOIN SALEPOINTS Where PRODUCTS.is_deleted = 0 AND SALEPOINTs.is_deleted = 0@
...
CREATE VIEW "v02" AS
 SELECT
	...
	sp.code as SPCODE from synonims LEFT JOIN SAlepoints as sp) 
	as FUL ON FUL.SID = PRICELIST.synonim_id 	
	LEFT JOIN V04 as VOR ON VOR.synonim_id = FUL.SID AND VOR.salepoint_code = FUl.SPCODE AND sended=0	
